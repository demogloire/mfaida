...
$(document).ready(function(){
	...
	// Datatable
	window.initDataTables = function (context) {
		if (!(window.jQuery && jQuery.fn && jQuery.fn.DataTable)) return;

		const $ctx = context ? jQuery(context) : jQuery(document);
		let $tables = $ctx.find('.datatable');
		if ($ctx.is('table.datatable')) $tables = $tables.add($ctx);
		if ($tables.length === 0) return;

		// If a previous init moved filter/length outside the wrapper,
		// those nodes can remain and appear duplicated after re-init.
		jQuery('#tableSearch .dataTables_filter, .search-input .dataTables_filter').remove();
		jQuery('#tableSearch .dataTables_length, .search-input .dataTables_length').remove();

		// Destroy then re-init to avoid duplicated search boxes / reinitialise errors.
		...
		$tables.each(function () {
			...
			// Ensure correct sizing after HTMX swaps/layout changes
			try {
				dt.columns.adjust();
			} catch (e) {}

			// Keep filter/length inside the wrapper to avoid duplicates on re-init.
		});
	};
